...
<form action="{{route('categoria.edit',$categoria->id)}}" method="POST" class="form-action" role="form">
    @method('PUT')
    @csrf
    <h1>Editando {{$categoria->nombre}}</h1>
    <div class="form-group col-xs-6 col-sm-6 col-md-6 col-lg-6">
        <label class="sr-only" for="">Nombre</label>
        <input type="text" class="form-control" id="" required name="nombre" placeholder="Nombre" value="{{$categoria->nombre}}">


    </div>

    <button type="submit" class="btn btn-primary btn-primary">Guardar cambios</button>
</form>
</div>
</div>
